...
const evidenceDir = resolve(process.env.BRIDGE_BLACKBOX_EVIDENCE_DIR || "spec/verification/evidence/relay-local-control-blackbox");
const reportDir = resolve(process.env.BRIDGE_BLACKBOX_REPORT_DIR || evidenceDir);
mkdirSync(evidenceDir, { recursive: true });
mkdirSync(reportDir, { recursive: true });

const temp = mkdtempSync(resolve(tmpdir(), "bridge-relay-local-control-blackbox-"));
  ...
  return new Promise((resolveRun) => {
    ...
    const child = spawn("cargo", ["run", "--quiet", "--manifest-path", "apps/desktop/Cargo.toml", "--", ...args], {
      env: {
        ...
        BRIDGE_DESKTOP_STATE: statePath,
        BRIDGE_ADAPTER_BRIDGE_DEMO_URL: adapter.url,
