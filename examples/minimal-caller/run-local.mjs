#!/usr/bin/env node
// Minimal Bridge caller — runnable locally against the in-memory Worker.
//
...
console.log("   front-end opens intent.deep_link to launch Bridge Desktop");
...
async function desktopClaim(token, body) {
  ...
  const response = await workerFetch(`${apiBase}${path}`, {
    ...
    headers: {
      ...
      "x-bridge-install-id": body.install_id,
    },
    body: JSON.stringify({
...
async function desktopConfirm(token, bearer, installIdValue) {
  ...
  const response = await workerFetch(`${apiBase}${path}`, {
    ...
    headers: {
      ...
      "x-bridge-local-client": "connector-cli",
      "x-bridge-install-id": installIdValue,
    },
