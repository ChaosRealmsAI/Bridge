...
一个最小的 Bridge 调用方示例，本地即可跑通完整链路。展示当前产品服务端 SDK 的端到端流程：
...
2. createConnectIntent(): token = pbi_…
   front-end opens intent.deep_link to launch Bridge Desktop
...
文件里 `desktopClaim` / `desktopConfirm` / `connectorReadAndReply` / `connector` 这些 helper **不是**调用方的活，它们模拟原生桌面端与本机连接器（用的是公开的 connector API）。生产环境里这部分由真实的 Bridge Desktop 完成。

Bridge 只负责授权、连接状态、HMAC delegated call 和 opaque relay 传输；业务协议、payload 加密和 key wrapping 都在产品侧。授权是用户的唯一决策（`active` / `paused` / 删除），连接全自动，调用方只读 `connected`。
