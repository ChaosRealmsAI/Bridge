...
pub fn show_session_at_path(
    ...
    let kind = source_kind_for_path(transcript_path, agent)?;
...
fn source_kind_for_path(path: &Path, agent: Option<&str>) -> Result<SourceKind, ShowError> {
    match agent {
        Some("codex") => return Ok(SourceKind::Codex),
        Some("claude") => return Ok(SourceKind::Claude),
        Some(other) => {
            return Err(ShowError::new(
                "invalid_agent",
