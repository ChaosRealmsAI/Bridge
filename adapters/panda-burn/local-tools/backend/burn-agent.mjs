...
import { createObserverCommands } from "./burn-agent-observer.mjs";
import { defaultBurnHome, stableHash } from "./burn-store-lib.mjs";
...
const observerCommands = createObserverCommands({
  backendAgentPath: fileURLToPath(import.meta.url),
  cleanText,
  coded,
  defaultBurnHome,
  discoverProfiles,
...
async function main() {
  ...
  if (command === "observer" || command === "observe") {
    const sub = args[1] || "status";
    if (sub === "status") return print(await observerCommands.observerStatus(options));
    if (sub === "sources" || sub === "source-roots") return print(await observerCommands.observerSources(options));
    if (sub === "deltas" || sub === "delta") {
      const action = args[2] || "list";
      if (action === "list" || action === "scan") return print(await observerCommands.observerDeltasList(options));
      throw coded("burn_agent_usage", `unknown observer deltas command: ${action}`);
    }
    if (sub === "watch") {
      const action = args[2] || "status";
      if (action === "start") return print(await observerCommands.observerWatchStart(options));
      if (action === "stop") return print(await observerCommands.observerWatchStop(options));
      if (action === "status") return print(await observerCommands.observerStatus(options));
      throw coded("burn_agent_usage", `unknown observer watch command: ${action}`);
    }
    if (sub === "daemon-run") return print(await observerCommands.observerDaemonRun(options));
    if (sub === "perf" || sub === "performance") return print(await observerCommands.observerPerf(options));
    throw coded("burn_agent_usage", `unknown observer command: ${sub}`);
  }
  if (command === "abnormal") {
    const sub = args[1] || "list";
    if (sub === "list") return print(await observerCommands.abnormalList(options));
    if (sub === "scan") return print(await observerCommands.abnormalScan(options));
    if (sub === "watch") {
      const action = args[2] || "status";
      if (action === "start") return print(await observerCommands.observerWatchStart(options));
      if (action === "stop") return print(await observerCommands.observerWatchStop(options));
      if (action === "status") return print(await observerCommands.observerStatus(options));
      throw coded("burn_agent_usage", `unknown abnormal watch command: ${action}`);
    }
    throw coded("burn_agent_usage", `unknown abnormal command: ${sub}`);
  }
...
function parse(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (["json", "deep", "json-stream", "quick", "live", "no-live", "latest", "dry-run", "include-suppressed", "force", "refresh-quota", "allow-token-spend", "confirm-token-spend", "once"].includes(key)) {
      options[key] = true;
...
Usage:
  ...
  burn agent observer status|sources|deltas list|watch start|watch stop|perf [--json]
  burn agent observer daemon-run [--once] [--interval-ms 1000] [--lease-ms 30000] [--json]
  burn agent abnormal list|scan|watch [--stability-ms 30000] [--no-response-ms 130000] [--json]
