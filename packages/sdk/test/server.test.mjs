...
assert.equal(calls[4].path, "/v1/products/example-product/delegated/authorization?device_id=dev_1");
...
assert.equal(calls[5].path, "/v1/products/example-product/delegated/relay/envelopes");
assert.equal(calls[5].init.method, "POST");
assert.equal(new Headers(calls[5].init.headers).get("x-bridge-device-id"), "dev_1");
...
assert.equal(calls[6].path, "/v1/products/example-product/delegated/connect-intents");
...
assert.equal(calls[7].path, "/v1/products/example-product/delegated/relay-key-bootstrap");
assert.equal(calls[7].init.method, "POST");
assert.equal(new Headers(calls[7].init.headers).get("x-bridge-device-id"), "dev_1");
...
assert.equal(waitCalls[0].path, "/v1/products/example-product/delegated/relay/envelopes?device_id=dev_1&channel_id=chan_1&after_seq=1");
assert.equal(waitCalls[0].init.method, "GET");
assert.equal(new Headers(waitCalls[0].init.headers).get("x-bridge-device-id"), "dev_1");
await waited.ack();
assert.equal(waitCalls[1].path, "/v1/products/example-product/delegated/relay/envelopes/env_reply_1/ack");
...
assert.equal(waitCalls[2].path, "/v1/products/example-product/delegated/relay/envelopes?device_id=dev_1&channel_id=chan_1&after_seq=2&limit=25&wait_ms=5000&include_acked=true");
...
assert.deepEqual(fallbackCalls, [
  "/v1/products/example-product/delegated/state",
  "/v1/products/example-product/delegated/status",
]);
...
const accountLevelServer = createBridgeServerClient({
  ...
  fetch: async (url, init) => {
    const parsed = new URL(url);
    accountLevelCalls.push({ path: `${parsed.pathname}${parsed.search}`, method: init.method, deviceHeader: new Headers(init.headers).get("x-bridge-device-id") });
...
assert.deepEqual(accountLevelCalls.map((c) => [c.method, c.path]), [
  ["PATCH", "/v1/products/example-product/delegated/authorization"],
  ["PATCH", "/v1/products/example-product/delegated/authorization"],
  ["DELETE", "/v1/products/example-product/delegated/authorization"],
]);
