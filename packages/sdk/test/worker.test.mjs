...
function assertSignedExactly(call, input) {
  ...
  assert.equal(call.init.headers["x-bridge-product-id"], "example-product");
  assert.equal(call.init.headers["x-bridge-user-id"], input.userId);
  assert.equal(call.init.headers["x-bridge-device-id"], input.deviceId);
  assert.equal(call.init.headers["x-bridge-request-timestamp"], timestamp);
  assert.equal(call.init.headers["x-bridge-request-nonce"], nonce);
  assert.equal(call.init.headers["x-bridge-body-sha256"], bodyHash);
  assert.equal(
    call.init.headers["x-bridge-signature"],
