...
assert.equal(bridgeDesktopInstallDefaults.macos.fileName, "bridge-macos.dmg");
...
assert.equal(
  ...
  "https://assets-bridge-test.chaos-realms.cc/downloads/bridge-macos.dmg",
);
assert.equal(
  bridgeDesktopInstallTarget({ assetBaseUrl: "https://cdn.example.test/" }).downloadUrl,
  "https://cdn.example.test/downloads/bridge-macos.dmg",
);
assert.equal(
  bridgeDesktopInstallTarget({ downloadUrl: "https://download.example.test/Bridge.dmg" }).downloadUrl,
  "https://download.example.test/Bridge.dmg",
);
assert.equal(bridgeDesktopInstallTarget().openUrl, "bridge://open");
assert.equal(bridgeDesktopInstallTarget().sha256.length, 64);
assert.equal(bridgeDesktopInstallTarget({ platform: "windows" }).fileName, "bridge-windows-x64.zip");
assert.equal(
...
const pendingIntentModel = bridgeDelegatedConnectIntentStatusModel({
  deep_link: "bridge://connect?intent=pbi_test&api=https%3A%2F%2Fapi.bridge.chaos-realms.cc",
...
assert.match(pendingIntentModel.deepLink, /^bridge:\/\/connect/);
...
const unauthPreflight = mockClient([
  { body: { ok: true, protocol: "bridge-protocol-v0.2" } },
...
assert.equal(install.openUrl, "bridge://open");
