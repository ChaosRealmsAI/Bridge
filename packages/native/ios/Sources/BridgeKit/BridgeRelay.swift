...
public enum BridgeError: Error, Sendable, CustomStringConvertible, LocalizedError {
    case notConfigured
...
public enum BridgeRelayClient {
    public static func call(
        ...
        guard let url = config.relayEnvelopeURL else { throw BridgeError.notConfigured }
        guard let key = Data(base64Encoded: config.relayKeyB64), key.count == 32 else { throw BridgeError.invalidRelayKey }
        ...
        guard (postResponse["ok"] as? Bool) == true else {
            throw BridgeError.invalidResponse("missing_response_envelope")
        }
        guard let channelId = envelope["channel_id"] as? String, !channelId.isEmpty else {
            throw BridgeError.invalidResponse("missing_channel_id")
        }
        ...
        throw BridgeError.adapterError("bridge_relay_timeout:Bridge relay timed out.")
    }
    ...
    private static func decodeRelayResult(_ responseEnvelope: [String: Any], key: Data) throws -> [String: Any] {
        if let direction = responseEnvelope["direction"] as? String, !direction.isEmpty, direction != "device_to_product" {
            throw BridgeError.invalidResponse("unexpected_response_direction:\(direction)")
        }
        ...
        if (result["ok"] as? Bool) == false {
            ...
            throw BridgeError.adapterError("\(code):\(message)")
        }
        return result
    }
    ...
    private static func requestJson(url: URL, method: String, body: [String: Any]?, timeoutSeconds: TimeInterval) async throws -> [String: Any] {
        ...
        guard status >= 200 && status < 300 else {
            throw BridgeError.adapterError("adapter_http_\(status):\(rawText)")
        }
...
public enum BridgeRelayCrypto {
    ...
    public static func decryptEnvelope(_ envelope: [String: Any], key: Data) throws -> [String: Any] {
        guard
            ...
            sealedData.count >= 16
        else { throw BridgeError.invalidResponse("invalid_envelope") }
    ...
    public static func jsonObject(_ data: Data) throws -> [String: Any] {
        guard let object = try JSONSerialization.jsonObject(with: data, options: []) as? [String: Any] else {
            throw BridgeError.invalidResponse("json_root_not_object")
        }
        return object
    }
    ...
    private static func decodedPayload(_ data: Data, envelope: [String: Any]) throws -> Data {
        ...
        default:
            throw BridgeError.invalidResponse("unsupported_content_encoding:\(encoding)")
        }
    }
    ...
    private static func gunzip(_ data: Data) throws -> Data {
        ...
        guard status == Z_OK else { throw BridgeError.invalidResponse("gzip_init_failed:\(status)") }
            ...
            repeat {
                ...
                try chunk.withUnsafeMutableBytes { outputRaw in
                    guard let outputBase = outputRaw.bindMemory(to: Bytef.self).baseAddress else {
                        throw BridgeError.invalidResponse("gzip_output_buffer_unavailable")
                    }
                    stream.next_out = outputBase
                    stream.avail_out = uInt(chunkSize)
                    status = inflate(&stream, Z_NO_FLUSH)
                    guard status == Z_OK || status == Z_STREAM_END else {
                        throw BridgeError.invalidResponse("gzip_inflate_failed:\(status)")
                    }
...
public enum BridgeRelayKeyBootstrap {
    ...
    public static func wrapRelayKeyForDesktop(relayKey: Data, exchange: [String: Any], aadText: String) throws -> [String: Any] {
        ...
        else { throw BridgeError.invalidResponse("invalid_relay_key_exchange") }
