...
async function serve(args) {
  ...
  console.log(`Bridge Server listening on ${localServerOrigin(server)}`);
...
async function pair(args) {
  ...
  const payload = await requestPairingToken({
    serverUrl,
    adminToken,
    deviceName: args.deviceName ?? env("BRIDGE_PAIRING_DEVICE_NAME") ?? "Bridge Desktop",
...
async function printStartupPairingToken(serverUrl, localOrigin) {
  ...
  const payload = await requestPairingTokenFromWorker({
    ...
    localOrigin,
    adminToken,
    deviceName: env("BRIDGE_PAIRING_DEVICE_NAME") ?? "Bridge Desktop",
