...
export function desktopReleaseManifestPath(options = {}) {
  ...
  const fileName = options.versioned
    ? `bridge-desktop-v${contract.version}.json`
    : "bridge-desktop-latest.json";
...
function assertDesktopReleaseShape(contract) {
  assert.equal(contract.schemaVersion, "2026-06-21.bridge.desktop-release.v1");
  assert.equal(contract.product, "bridge-desktop");
  assert.equal(contract.appName, "Bridge");
  assert.equal(contract.bundleIdentifier, "ai.chaosrealms.bridge");
  assert.equal(contract.protocol, "bridge");
...
function assertTarget(contract, id, expected) {
  ...
  assert.ok(target.fileName.startsWith("bridge-"), `${id} stable file must use bridge prefix`);
...
async function prepareReleaseManifest() {
  ...
  const manifest = {
    schemaVersion: "2026-06-21.bridge.desktop-release-manifest.v1",
