...
export const bridgeManagedAdapterNodePackages = [
  { name: "@panda-bridge/protocol", source: "packages/protocol" },
  { name: "@panda-bridge/sdk", source: "packages/sdk" },
  { name: "@panda-bridge/adapter-sdk", source: "packages/adapter-sdk" },
];
...
export function prepareManagedAdapterSources(sourceRoot) {
  if (process.env.PANDA_BRIDGE_SKIP_MANAGED_ADAPTER_PREPARE === "1") return [];
  const prepared = [];
...
function copyDependencyTree(name, context) {
  if (!name || name.startsWith("@panda-bridge/") || context.seen.has(name)) return;
