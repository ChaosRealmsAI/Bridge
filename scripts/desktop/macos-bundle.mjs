...
export const binaryName = "bridge-desktop";
export const iconFileName = "Bridge.icns";
...
export const entitlementsPath = resolve("dist/desktop/macos/bridge.entitlements.plist");
...
// Sign the bundle. With a real Developer ID identity (BRIDGE_CODESIGN_IDENTITY,
...
export function signAppBundle(appPath, options = {}) {
  const identity = options.identity ?? process.env.BRIDGE_CODESIGN_IDENTITY ?? "-";
  ...
  if (adhoc) {
    console.warn(
      "[codesign] ad-hoc signing (DEV ONLY). This build will NOT pass Gatekeeper, " +
        ...
        "Set BRIDGE_CODESIGN_IDENTITY to a Developer ID Application identity to ship.",
    );
...
// either BRIDGE_NOTARY_PROFILE (a `xcrun notarytool store-credentials` profile)
// or BRIDGE_NOTARY_APPLE_ID + _TEAM_ID + _PASSWORD (an app-specific password).
...
export function notarizeApp({ appPath, dmgPath }) {
  ...
  const profile = process.env.BRIDGE_NOTARY_PROFILE;
  const appleId = process.env.BRIDGE_NOTARY_APPLE_ID;
  const teamId = process.env.BRIDGE_NOTARY_TEAM_ID;
  const password = process.env.BRIDGE_NOTARY_PASSWORD;
  ...
  if (!credArgs) {
    console.warn(
      "[notarize] skipped: set BRIDGE_NOTARY_PROFILE (from `xcrun notarytool store-credentials`) " +
        "or BRIDGE_NOTARY_APPLE_ID/_TEAM_ID/_PASSWORD to notarize for distribution.",
    );
