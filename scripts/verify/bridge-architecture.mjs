...
function assertNoActiveLegacySurface(file, text) {
  ...
  if (text.includes("headless-poll")) {
    for (const marker of ["BRIDGE_FAKE_CODEX", "本机 Codex", "Bridge fixture reply", "jobs: {}", "codex: {}"]) {
...
function fileBudgets() {
  const budgets = [
    ...
    {
      file: "apps/desktop/assets/bridge-icon.svg",
      maxLines: 120,
...
function assertDesktopUiAssetSplit() {
  ...
  assert.ok(index.includes("__BRIDGE_DESKTOP_CSS__"), "Desktop UI index must keep a CSS compile-time placeholder");
  assert.ok(index.includes("__BRIDGE_DESKTOP_ABOUT_JS__"), "Desktop UI index must keep an About JS compile-time placeholder");
  assert.ok(index.includes("__BRIDGE_DESKTOP_JS__"), "Desktop UI index must keep a JS compile-time placeholder");
