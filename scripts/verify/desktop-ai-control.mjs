...
const temp = mkdtempSync(resolve(tmpdir(), "bridge-desktop-ai-control-"));
...
try {
  ...
  const desktopBinary = resolve("apps/desktop/target/debug/bridge-desktop");
  desktop = spawn(desktopBinary, [], {
    cwd: resolve("."),
    env: {
      ...process.env,
      HOME: homePath,
      BRIDGE_DESKTOP_STATE: desktopStatePath,
      BRIDGE_SKIP_KEYCHAIN: "1",
      BRIDGE_VERIFY: "1",
      BRIDGE_VERIFY_CONTROL_STATE: verifyControlPath,
    },
  ...
  step("ai-control-started", {
    action: "Start Desktop with BRIDGE_VERIFY=1",
...
async function controlJson(control, method, path, body = null, options = {}) {
  const response = await fetch(`${control.base_url}${path}`, {
    ...
    headers: {
      ...
      "x-bridge-verify-token": control.token,
    },
