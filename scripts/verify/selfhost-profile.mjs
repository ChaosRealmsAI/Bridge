...
const temp = mkdtempSync(resolve(tmpdir(), "bridge-selfhost-profile-"));
  ...
  try {
    ...
    await captureDesktopUi(browser, "trust-dialog-preview.png", afterPreview, async (page) => {
      await page.evaluate(({ api, previewPayload }) => {
        window.__bridgePreview = previewPayload;
        window.Bridge.receive({
          type: "event",
          event: "deep_link",
          url: `bridge://connect?intent=visual-preview&api=${encodeURIComponent(api)}`,
        });
...
async function captureDesktopUi(browser, fileName, statusPayload, beforeShot) {
  ...
  await page.addInitScript((status) => {
    window.__bridgeStatus = status;
    window.__bridgePreview = null;
    window.ipc = {
      postMessage(raw) {
        const req = JSON.parse(raw);
        const reply = (ok, result, error) => setTimeout(() => {
          window.Bridge.receive({ type: "response", id: req.id, ok, result, error });
        }, 10);
        if (req.command === "status") return reply(true, window.__bridgeStatus);
        if (req.command === "settings") return reply(true, window.__bridgeStatus.settings || {});
        if (req.command === "preview_intent") return reply(true, window.__bridgePreview || {
        ...
        if (req.command === "select_cloud_profile" || req.command === "update_settings" || req.command === "refresh_cloud_profile" || req.command === "pair_selfhost_profile") return reply(true, window.__bridgeStatus.settings || {});
...
function writeCompiledDesktopUiHtml() {
  ...
  const html = indexSource
    .replace("__BRIDGE_DESKTOP_CSS__", cssSource)
    .replace("__BRIDGE_DESKTOP_ABOUT_JS__", aboutSource)
    .replace("__BRIDGE_DESKTOP_JS__", jsSource)
    .replace(RAW_EMAIL_LIKE_GLOBAL_PATTERN, "[redacted-email]")
    .replace(CONTACT_PROFILE_URL_PATTERN, "[redacted-contact-url]");
  assert.equal(html.includes("__BRIDGE_DESKTOP_CSS__"), false, "compiled selfhost UI HTML must embed CSS");
  assert.equal(html.includes("__BRIDGE_DESKTOP_ABOUT_JS__"), false, "compiled selfhost UI HTML must embed About JS");
  assert.equal(html.includes("__BRIDGE_DESKTOP_JS__"), false, "compiled selfhost UI HTML must embed JS");
...
async function createPairingToken(apiBase, deviceName) {
  const response = await fetch(`${apiBase}/v1/selfhost/pairing-token`, {
    ...
    headers: {
      ...
      "x-bridge-selfhost-admin-token": SELFHOST_ADMIN_TOKEN,
    },
  ...
  return new Promise((resolveChild) => {
    const child = spawn("cargo", ["run", "--quiet", "--manifest-path", "apps/desktop/Cargo.toml", "--", ...args], {
      env: {
        ...
        BRIDGE_ALLOW_HEADLESS_CONNECT: "1",
        BRIDGE_DESKTOP_STATE: activeStatePath,
        BRIDGE_ADAPTER_ACME_DEMO_URL: adapter.url,
        BRIDGE_ADAPTER_URL: adapter.url,
        BRIDGE_SKIP_KEYCHAIN: "1",
      },
