...
try {
  ...
  writeFileSync(join(home, ".codex/sessions/2026/06/19/rollout-codex-error.jsonl"), [
    JSON.stringify({
      timestamp: "2026-06-19T00:00:00.000Z",
      type: "session_meta",
      payload: { id: "codex-error", cwd: project },
    }),
    JSON.stringify({
      timestamp: "2026-06-19T00:00:01.000Z",
      type: "event_msg",
      payload: { type: "error", message: "API Error: The socket connection was closed unexpectedly. For more information, pass verbose: true in the second argument to fetch()" },
    }),
    "",
  ].join("\n"));
...
`);
  ...
  const observerSources = await call("burn.agent.observer.sources", {}, context);
  assert.equal(observerSources.ok, true, "observer sources ok");
  assert.equal(observerSources.data.schema, "burn.agent.observer.sources.v1", "observer sources schema");
  assert.ok(observerSources.data.counts.source_roots >= 2, "observer source roots should include Codex and Claude");

  const observerDeltas = await call("burn.agent.observer.deltas.list", { dry_run: true }, context);
  assert.equal(observerDeltas.ok, true, "observer deltas ok");
  assert.equal(observerDeltas.data.schema, "burn.agent.observer.deltas.v1", "observer deltas schema");
  assert.ok(observerDeltas.data.counts.session_added >= 1, "observer initial scan should see JSONL sessions");

  const abnormal = await call("burn.agent.abnormal.list", { stability_ms: 0, include_suppressed: true }, context);
  assert.equal(abnormal.ok, true, "abnormal list ok");
  assert.equal(abnormal.data.schema, "burn.agent.abnormal.list.v1", "abnormal list schema");
  assert.ok(abnormal.data.incidents.some((item) => item.marker.id === "socket_connection_closed"), "socket closed incident should be detected");
