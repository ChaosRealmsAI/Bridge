...
assert.equal(compiledHtml.includes("__BRIDGE_DESKTOP_CSS__"), false, "compiled smoke HTML must embed CSS");
assert.equal(compiledHtml.includes("__BRIDGE_DESKTOP_ABOUT_JS__"), false, "compiled smoke HTML must embed About JS");
assert.equal(compiledHtml.includes("__BRIDGE_DESKTOP_JS__"), false, "compiled smoke HTML must embed JS");
...
const duplicateRecheck = await inspectScenario("duplicate-recheck-guard", {
  ...
  async action(page) {
    ...
    await page.evaluate(() => {
      ...
      const original = window.Bridge.call.bind(window.Bridge);
      window.Bridge.call = (command, params = {}) => {
...
const probeTimeoutRecovery = await inspectScenario("probe-timeout-recovery", {
  ...
  async action(page) {
    ...
    await page.evaluate(() => {
      ...
      ui.serverBusy = {};
      ui.serverProbeBackoff = {};
      window.__BRIDGE_TEST_PROBE_TIMEOUT_MS = 80;
      window.__probeCallCount = 0;
      const original = window.Bridge.call.bind(window.Bridge);
      window.Bridge.call = (command, params = {}) => {
        if (command === "refresh_cloud_profile") {
