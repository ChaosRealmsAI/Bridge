...
const temp = mkdtempSync(join(tmpdir(), "panda-bridge-managed-adapters-"));
...
function adapterRuntimeDependencies(adapters) {
  ...
  for (const adapter of adapters) {
    ...
    for (const dependencyName of Object.keys(packageJson.dependencies || {})) {
      if (dependencyName.startsWith("@panda-bridge/")) continue;
...
async function assertNodeAdapterStarts(adapter) {
  ...
  const child = spawn("node", [entry, ...(manifest.runtime.args || [])], {
    ...
    env: {
      ...process.env,
      PANDA_BRIDGE_RELAY_KEY_B64: keyB64,
      HOME: join(temp, "home"),
    },
