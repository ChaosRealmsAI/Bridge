...
const xwinEnabled = args.has("--xwin") || process.env.BRIDGE_WINDOWS_XWIN === "1";
...
if (process.platform === "win32") {
  ...
  checks.push(runCheck("Windows URL protocol registry query", "reg.exe", [
    "query",
    "HKCU\\Software\\Classes\\bridge\\shell\\open\\command",
  ], {
    expectStdout: "Bridge.exe",
  }));
...
function windowsPackageArtifactCheck() {
  ...
  const manifest = resolve("dist/desktop/windows/Bridge/manifest.json");
  const exe = resolve("dist/desktop/windows/Bridge/Bridge.exe");
  const install = resolve("dist/desktop/windows/Bridge/Install.ps1");
...
function staticSourceCheck() {
  ...
  const required = [
    { file: "apps/desktop/src/window.rs", marker: "Software\\Classes\\bridge" },
    ...
    { file: "scripts/desktop/package-windows.mjs", marker: "Bridge.exe" },
...
function releaseBinaryPath() {
  return resolve("apps/desktop/target/release/bridge-desktop.exe");
}

function installScriptPath() {
  return resolve("dist/desktop/windows/Bridge/Install.ps1");
}

function uninstallScriptPath() {
  return resolve("dist/desktop/windows/Bridge/Uninstall.ps1");
}
