...
const tempDir = mkdtempSync(resolve(tmpdir(), "bridge-file-store-"));
...
try {
  ...
  }, "", { "x-bridge-selfhost-admin-token": adminToken, origin: null });
  ...
  const confirmed = await desktopApi("POST", `/v1/connect-intents/${encodeURIComponent(intent.token)}/confirm`, {
    confirmed: true,
  }, claimed.device_token, { "x-bridge-install-id": installId });
  ...
  await desktopApi("POST", "/v1/connectors/heartbeat", {
    ...
    capabilities: { relay: ["relay.envelope", "relay.ack"] },
    local_state: { device_info: safeDeviceInfo() },
  }, deviceToken, { "x-bridge-install-id": installId });
  summary.authorization.confirmed = true;

  await restart("runtime-d-before-state-read");
  const state = await api("GET", `/v1/bridge/state?product_id=${encodeURIComponent(productId)}`, null, deviceToken, { "x-bridge-install-id": installId });
  ...
  const inbox = await desktopApi("GET", "/v1/connectors/relay/envelopes", null, deviceToken, { "x-bridge-install-id": installId });
  assert.equal(inbox.items.some((item) => item.id === created.envelope.id), true);
  await desktopApi("POST", `/v1/connectors/relay/envelopes/${created.envelope.id}/ack`, {}, deviceToken, { "x-bridge-install-id": installId });
  ...
  const concurrentInbox = await desktopApi("GET", "/v1/connectors/relay/envelopes", null, deviceToken, { "x-bridge-install-id": installId });
