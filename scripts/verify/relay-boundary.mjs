...
assert.ok(desktopProduction.includes("BRIDGE_ADAPTER_URL"), "Desktop must use product adapter endpoint env");
...
for (const source of [macosBundle, windowsPackage, managedAdaptersPackageHelper]) {
  for (const marker of ["BRIDGE_BURN_ADAPTER_DIR", ["..", "syllo"].join("/"), "burn_manifest", "panda-burn"]) {
...
for (const source of [macosBundle, windowsPackage]) {
  assert.ok(source.includes("BRIDGE_MANAGED_ADAPTERS_DIR"), "desktop package scripts must use the generic managed adapter directory env");
}
assert.ok(
...
const productDocsPath = new URL("../../spec/L4/reference-materials/docs/product-integration.md", import.meta.url);
if (existsSync(productDocsPath)) {
  const productDocs = readFileSync(productDocsPath, "utf8");
...
const selfHostingDocsPath = new URL("../../spec/L4/reference-materials/docs/self-hosting.md", import.meta.url);
if (existsSync(selfHostingDocsPath)) {
  const selfHostingDocs = readFileSync(selfHostingDocsPath, "utf8");
  for (const marker of ["BRIDGE_PRODUCT_REGISTRY_JSON", "BRIDGE_ADAPTER_ACME_DEMO_URL", "npm run verify:selfhost-profile"]) {
