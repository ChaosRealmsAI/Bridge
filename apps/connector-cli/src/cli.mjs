...
async function claimIntent() {
  ...
  if (!args.yes && !args.assume_yes) {
    ...
    console.error(`Claiming ${preview.connect_intent?.product_id || "Panda Bridge"} intent for ${deviceName()}.`);
  }
...
function installId() {
  return args["install-id"] || process.env.PANDA_BRIDGE_INSTALL_ID || `connector-${hostname()}-${platform()}`;
}
...
function printHelp() {
  console.log(`
panda-bridge connector relay CLI

Usage:
  panda-bridge doctor [--api URL] [--state PATH]
  panda-bridge claim-intent --intent TOKEN [--yes] [--api URL] [--state PATH]
  panda-bridge confirm-intent --intent TOKEN [--state PATH]
  panda-bridge heartbeat [--state PATH]
  panda-bridge poll-relay [--product-id ID] [--channel-id ID] [--wait-ms 3000] [--state PATH]
  panda-bridge ack-relay --envelope-id ID [--state PATH]
  panda-bridge send-relay --product-id ID --channel-id ID --seq 2 --ciphertext B64 --aad B64 --nonce B64 --sender-key-id ID --recipient-key-id ID [--state PATH]
