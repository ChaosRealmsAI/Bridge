...
const PRODUCT_PROFILES = Object.freeze({
  "bridge-demo": {
    ...
    chatSubtitle: "Sign in, connect Bridge Desktop, and inspect relay readiness.",
...
nodes.composer?.addEventListener("submit", (event) => {
  ...
  addMessage("assistant", "Bridge Web demo does not define product payloads. Use @bridge/sdk relay.create/createCall from the product app, and keep encryption in your Product Adapter.");
});
...
async function runPrimaryBridgeAction() {
  ...
  try {
    ...
    addMessage("assistant", "Connection intent created. Open the deep link on Bridge Desktop, approve it, then refresh.");
    render();
  } catch (error) {
    addMessage("assistant", formatError(error), "error");
  } finally {
...
function renderBridgePanel(account) {
  ...
  if (connected) {
    ...
    setText(nodes.bridgeCopy, `${device?.name || "Bridge Desktop"} is online. Product payloads must stay encrypted.`);
    ...
    setText(nodes.bridgeCopy, "Open Bridge Desktop with the deep link below, approve, then refresh this page.");
  } else {
    setText(nodes.bridgeTitle, "Approval required");
    setText(nodes.bridgeCopy, "Create a connection intent and approve it from Bridge Desktop.");
  }
...
function renderBridgePrimary(connected) {
  ...
  if (state.intent?.deep_link) {
    nodes.bridgePrimary.append(buttonLink(state.intent.deep_link, "Open Bridge"));
