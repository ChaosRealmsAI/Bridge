...
function render(){
  ...
  if(aboutTab){document.getElementById("aboutIco").innerHTML=window.BridgeAbout.icon;document.getElementById("aboutLabel").textContent=t("aboutAuthor");aboutTab.setAttribute("aria-current",ui.view==="about"?"true":"false")}
  document.getElementById("engine").innerHTML=engineHtml();
  const pane=document.getElementById("pane");
  pane.innerHTML=ui.view==="about"?window.BridgeAbout.html():ui.view==="settings"?settingsHtml():productHtml(productById(ui.selected)||ui.products[0]);
  document.getElementById("modalRoot").innerHTML=modalHtml();
  applyTheme();
  if(ui.view==="about")window.BridgeAbout.init(lang());
...
async function toggleAuth(productId,index){
  ...
  try{await window.Bridge.call("toggle_authorization",{product_id:productId,account:a.id||a.email});toast(a.authorized==="paused"?t("resumed"):t("paused"));await refresh()}catch(e){showError(e)}
}
async function toggleConnection(productId,index){const p=productById(productId),a=p?.accounts?.[index];if(!a||a.authorized==="paused")return;const next=a.connection_enabled===false;try{await window.Bridge.call("toggle_connection",{product_id:productId,account:a.id||a.email,enabled:next});toast(next?t("connectionResumed"):t("connectionPaused"));await refresh()}catch(e){showError(e)}}
async function removeAccount(productId,index){
  ...
  closePop();
  try{await window.Bridge.call("remove_authorization",{product_id:productId,account_id:a.id||a.email,account:a.id||a.email});toast(t("removed"));await refresh()}catch(e){showError(e);await refresh()}
}
async function openProduct(productId){try{await window.Bridge.call("open_web",{product_id:productId});toast(t("opened"))}catch(e){showError(e)}}
...
async function addCloudProfile(){
  ...
  try{ui.settings=await window.Bridge.call("add_cloud_profile",{api});toast(t("serverAdded"));await refresh()}catch(e){showError(e)}
}
function openSelfhostHelp(){if(SELFHOST_HELP_URL)window.Bridge.call("open_web",{url:SELFHOST_HELP_URL}).catch(()=>{})}
...
async function submitPairServer(){
  ...
  try{
    ui.settings=await window.Bridge.call("pair_selfhost_profile",{api,token,name:"My Server"});
...
async function probeServer(ev,id,opts){
  ...
  try{
    const settings=await window.Bridge.call("refresh_cloud_profile",{profile_id:id});if(!serverProbeCurrent(id,token))return;ui.settings=settings;
...
function serverProbeTimeoutMs(){const n=Number(window.__BRIDGE_TEST_PROBE_TIMEOUT_MS||SERVER_PROBE_UI_TIMEOUT_MS);return Number.isFinite(n)&&n>0?n:SERVER_PROBE_UI_TIMEOUT_MS}
...
async function refreshCloudProfile(id){
  try{ui.settings=await window.Bridge.call("refresh_cloud_profile",{profile_id:id});toast(t("serverRefreshed"));await refresh()}catch(e){showError(e)}
}
async function removeCloudProfile(id){
  try{ui.settings=await window.Bridge.call("remove_cloud_profile",{profile_id:id});toast(t("serverRemoved"));await refresh()}catch(e){showError(e)}
}
async function refresh(){
  const status=await window.Bridge.call("status");
...
async function handleDeepLink(raw){
  ...
  const pre=await window.Bridge.call("preview_intent",{api,intent});
...
async function allowIntent(){
  ...
  render();
  try{
    const pending=await window.Bridge.call("claim_intent_preview",{api:p.api,intent:p.intent,device_name:localDeviceInfo()?.display_name||`Bridge ${navigator.platform||"Desktop"}`});
    await window.Bridge.call("confirm_pending_intent",{pending_id:pending.pending_id,intent:p.intent});
    ui.pending=null;ui.selected=p.select;ui.view="product";render();await window.Bridge.call("start_worker");await refresh()
  }catch(e){
...
function installFallback(){
  ...
  window.ipc={postMessage(raw){const req=JSON.parse(raw);const reply=(ok,result,error,delay=50)=>setTimeout(()=>window.Bridge.receive({type:"response",id:req.id,ok,result,error}),delay);
  ...
  if(new URLSearchParams(location.search).get("sheet"))setTimeout(()=>window.Bridge.receive({type:"event",event:"deep_link",url:"bridge://connect?intent=demo&api=https://api.bridge.chaos-realms.cc"}),120);
}
if(!window.ipc)installFallback();
applyTheme();
render();
refresh()
  .then(status=>{if(ui.view==="settings")probeAllServers();if((status.products||[]).some(p=>(p.accounts||[]).some(a=>a.authorized==="active"&&a.connection_enabled!==false)))window.Bridge.call("start_worker").catch(()=>{})})
