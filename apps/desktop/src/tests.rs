...
mod tests {
    ...
    fn reset_credentials_env() {
        env::remove_var("BRIDGE_DESKTOP_STATE");
        env::remove_var("BRIDGE_DESKTOP_STATE_DIR");
        env::remove_var("BRIDGE_USE_KEYCHAIN");
        env::remove_var("BRIDGE_SKIP_KEYCHAIN");
        env::remove_var("BRIDGE_SKIP_REMOTE_REVOKE");
    }
    ...
    fn credentials_default_to_private_fallback_file_without_keychain() {
        ...
        env::set_var("BRIDGE_SKIP_KEYCHAIN", "1");
        let old_home = env::var_os("HOME");
        let old_userprofile = env::var_os("USERPROFILE");
        let home = env::temp_dir().join(format!(
            "bridge-credentials-test-{}-{}",
            std::process::id(),
            unix_seconds()
        ));
        ...
        assert!(
            !path.starts_with(home.join(".bridge")),
            "new credentials writes must use the platform app data state directory"
        );
        #[cfg(target_os = "macos")]
        assert!(
            path.ends_with("Library/Application Support/Bridge/state/desktop-connector.json")
        );
        #[cfg(unix)]
        {
    ...
    fn credentials_migrate_legacy_hidden_home_state_to_app_data() {
        ...
        let old_userprofile = env::var_os("USERPROFILE");
        let home = env::temp_dir().join(format!(
            "bridge-legacy-credentials-test-{}-{}",
            std::process::id(),
            unix_seconds()
        ));
        let _ = fs::remove_dir_all(&home);
        fs::create_dir_all(home.join(".bridge")).unwrap();
        env::set_var("HOME", &home);
        env::remove_var("USERPROFILE");

        let credentials = test_credentials(vec!["relay.envelope", "relay.ack"]);
        fs::write(
            home.join(".bridge/desktop-connector.json"),
        ...
        assert!(!migrated_path.starts_with(home.join(".bridge")));
    ...
    fn keychain_default_depends_on_build_and_env_overrides() {
        ...
        env::set_var("BRIDGE_USE_KEYCHAIN", "1");
        assert!(
            keychain_enabled(),
    ...
    fn explicit_desktop_state_does_not_chmod_external_parent() {
        ...
        env::set_var("BRIDGE_DESKTOP_STATE", &state);
    ...
    fn cloud_profile_migrates_old_api_base_and_keeps_official_profile() {
        ...
        let old_home = env::var_os("HOME");
        let old_userprofile = env::var_os("USERPROFILE");
        let home = with_settings_home("bridge-settings-migration");
        let api = "http://local.test:8787";
        fs::write(
            home.join(".bridge/desktop-settings.json"),
    ...
    fn add_cloud_profile_rejects_invalid_diagnostics_without_saving() {
        ...
        let old_home = env::var_os("HOME");
        let old_userprofile = env::var_os("USERPROFILE");
        let home = with_settings_home("bridge-invalid-profile");
    ...
    fn refresh_cloud_profile_persists_probe_failure_marker() {
        ...
        let old_home = env::var_os("HOME");
        let old_userprofile = env::var_os("USERPROFILE");
        let home = with_settings_home("bridge-refresh-profile-failure");
        let (api, server) = start_profile_server(json!({
            "ok": true,
            "protocol": "not-bridge",
            "api_base": "http://127.0.0.1:1",
    ...
    fn open_web_uses_fixed_product_catalog_under_selected_server_profile() {
        ...
        let old_home = env::var_os("HOME");
        let old_userprofile = env::var_os("USERPROFILE");
        let home = with_settings_home("bridge-open-web-profile");
        let api = "http://local.test:8787";
        let mut settings = default_settings();
        let profile = CloudProfile {
            id: profile_id_for_api(api),
    ...
    fn test_pending_intent_claim() -> PendingIntentClaim {
        ...
        PendingIntentClaim {
            ...
            preview: IntentPreview {
                ...
                capabilities: vec!["relay.envelope".to_string(), "relay.ack".to_string()],
                local_policy: policy,
                device_name: "Bridge Desktop".to_string(),
    ...
    fn authorization_toggle_pauses_and_restores_account_product() {
        ...
        let state_path = env::temp_dir().join(format!(
            "bridge-toggle-test-{}.json",
            next_event_seq()
        ));
        env::set_var("BRIDGE_DESKTOP_STATE", &state_path);
        let credentials = test_credentials(vec!["relay.envelope", "relay.ack"]);
        save_credentials(&credentials).unwrap();
    ...
    fn connection_toggle_disables_local_relay_without_pausing_authorization() {
        ...
        reset_credentials_env();
        let state_path = env::temp_dir().join(format!(
            "bridge-connection-toggle-test-{}.json",
            next_event_seq()
        ));
        env::set_var("BRIDGE_DESKTOP_STATE", &state_path);
        let mut credentials = test_credentials(vec!["relay.envelope", "relay.ack"]);
    ...
    fn remove_authorization_deletes_local_account_product() {
        ...
        reset_credentials_env();
        let state_path = env::temp_dir().join(format!(
            "bridge-remove-test-{}.json",
            next_event_seq()
        ));
        env::set_var("BRIDGE_DESKTOP_STATE", &state_path);
        env::set_var("BRIDGE_SKIP_REMOTE_REVOKE", "1");
        let mut credentials = test_credentials(vec!["relay.envelope", "relay.ack"]);
    ...
    fn update_settings_persists_switches_and_manages_launch_agent() {
        ...
        env::set_var("BRIDGE_SKIP_KEYCHAIN", "1");
        let old_home = env::var_os("HOME");
        let home = env::temp_dir().join(format!("bridge-settings-test-{}", next_event_seq()));
        let _ = fs::remove_dir_all(&home);
        fs::create_dir_all(&home).unwrap();
        env::set_var("HOME", &home);
        ...
        #[cfg(target_os = "macos")]
        {
            let plist = home.join("Library/LaunchAgents/ai.chaosrealms.bridge.plist");
            assert!(
                plist.exists(),
                "enabling launch_at_login should write the LaunchAgent"
            );
            let text = fs::read_to_string(&plist).unwrap();
            assert!(text.contains("ai.chaosrealms.bridge"));
        ...
        assert_eq!(
            windows_registry_command_for_exe(Path::new(
                r"C:\Users\Ada Lovelace\AppData\Local\Bridge\Bridge.exe"
            )),
            r#""C:\Users\Ada Lovelace\AppData\Local\Bridge\Bridge.exe""#
        );
    ...
    fn selected_profile_server_status_reports_successful_probe_latency() {
        ...
        let old_home = env::var_os("HOME");
        let old_userprofile = env::var_os("USERPROFILE");
        let home = with_settings_home("bridge-profile-probe-latency");
        let (api, server) = start_profile_server(json!({
            "ok": true,
            "protocol": BRIDGE_PROTOCOL_VERSION,
            "api_base": "__API__",
    ...
    fn heartbeat_interval_defaults_to_thirty_seconds_and_allows_env_override() {
        let _guard = ENV_LOCK.lock().unwrap();
        env::remove_var("BRIDGE_HEARTBEAT_INTERVAL_MS");
        assert_eq!(heartbeat_interval_ms(), 30_000);
        env::set_var("BRIDGE_HEARTBEAT_INTERVAL_MS", "1234");
    ...
    fn local_device_info_selected_profile_status_and_local_state_are_sanitized() {
        ...
        let old_home = env::var_os("HOME");
        let old_userprofile = env::var_os("USERPROFILE");
        let old_state = env::var_os("BRIDGE_DESKTOP_STATE");
        let old_state_dir = env::var_os("BRIDGE_DESKTOP_STATE_DIR");
        reset_credentials_env();
        let home = with_settings_home("bridge-local-device-info");
        let old_install_file = env::var_os("BRIDGE_INSTALL_ID_FILE");
        let old_seed = env::var_os("BRIDGE_INSTALL_ID_SEED");
        let old_name = env::var_os("BRIDGE_COMPUTER_NAME");
        let old_model = env::var_os("BRIDGE_DEVICE_MODEL");
        let path = env::temp_dir().join(format!(
            "bridge-install-id-test-{}-{}",
            std::process::id(),
            unix_seconds()
        ));
        let _ = fs::remove_file(&path);
        env::set_var("BRIDGE_INSTALL_ID_FILE", &path);
        env::set_var("BRIDGE_INSTALL_ID_SEED", "stable-test-seed");
        env::set_var("BRIDGE_COMPUTER_NAME", "Alice MacBook");
        env::set_var("BRIDGE_DEVICE_MODEL", "MacBookPro18,3");
        ...
        env::set_var("BRIDGE_COMPUTER_NAME", "/Users/private");
        assert_ne!(local_device_info().display_name, "Users private");
        env::set_var("BRIDGE_COMPUTER_NAME", "192.168.1.44");
        assert_ne!(local_device_info().display_name, "192.168.1.44");
        env::set_var("BRIDGE_DEVICE_MODEL", "00:11:22:33:44:55");
        ...
        restore_env_var("HOME", old_home);
        restore_env_var("USERPROFILE", old_userprofile);
        restore_env_var("BRIDGE_DESKTOP_STATE", old_state);
        restore_env_var("BRIDGE_DESKTOP_STATE_DIR", old_state_dir);
        restore_env_var("BRIDGE_INSTALL_ID_FILE", old_install_file);
        restore_env_var("BRIDGE_INSTALL_ID_SEED", old_seed);
        restore_env_var("BRIDGE_COMPUTER_NAME", old_name);
        restore_env_var("BRIDGE_DEVICE_MODEL", old_model);
        let _ = fs::remove_file(path);
        let _ = fs::remove_dir_all(home);
    }
    ...
    fn managed_adapter_manifest_starts_node_runtime_and_returns_endpoint() {
        ...
        env::remove_var("BRIDGE_ADAPTER_PANDA_BURN_URL");
        env::remove_var("BRIDGE_ADAPTER_URL");
        let suffix = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap()
            .as_millis();
        let tmp = env::temp_dir().join(format!("bridge-managed-adapter-{suffix}"));
...
"#,
        ...
        .unwrap();
        env::set_var("BRIDGE_MANAGED_ADAPTERS_DIR", tmp.join("adapters"));
        ...
        env::remove_var("BRIDGE_MANAGED_ADAPTERS_DIR");
    ...
    fn desktop_ui_html_embeds_split_assets() {
        ...
        assert!(!html.contains("__BRIDGE_DESKTOP_CSS__"));
        assert!(!html.contains("__BRIDGE_DESKTOP_ABOUT_JS__"));
        assert!(!html.contains("__BRIDGE_DESKTOP_JS__"));
        assert!(html.contains(".win{"));
        assert!(html.contains("window.BridgeAbout"));
        assert!(html.contains("window.Bridge"));
