...
<dict>\n\
    <key>Label</key>\n\
    <string>ai.chaosrealms.bridge</string>\n\
...
pub(crate) fn apply_launch_at_login(enabled: bool) -> Result<(), String> {
    #[cfg(target_os = "macos")]
    {
        let path = home_dir()?.join("Library/LaunchAgents/ai.chaosrealms.bridge.plist");
        if !enabled {
            if path.exists() {
...
pub(crate) fn apply_windows_launch_at_login(enabled: bool) -> Result<(), String> {
    ...
    if !enabled {
        let _ = run_key.remove_value("Bridge");
        return Ok(());
    }
    let exe = env::current_exe().map_err(|error| error.to_string())?;
    run_key
        .set_string("Bridge", windows_registry_command_for_exe(&exe))
        .map_err(|error| error.to_string())
}
...
pub(crate) fn should_apply_launch_at_login_on_startup() -> bool {
    ...
    if cfg!(target_os = "windows") {
        return !cfg!(debug_assertions) && !env_flag("BRIDGE_DISABLE_STARTUP_APPLY");
    }
    false
}
...
pub(crate) fn should_register_windows_url_scheme_on_startup() -> bool {
    !cfg!(debug_assertions) || env_flag("BRIDGE_REGISTER_URL_SCHEME")
}
