...
pub(crate) fn public_fingerprint_for_install_id(install_id: &str) -> String {
    let digest =
        Sha256::digest(format!("panda-bridge-local-device-info-v1:{install_id}").as_bytes());
...
fn local_install_identity() -> String {
    if let Ok(path) = env::var("PANDA_BRIDGE_INSTALL_ID_FILE") {
        return read_or_create_install_identity(PathBuf::from(path));
    }
    let path = state_dir()
        .unwrap_or_else(|_| env::temp_dir().join("panda-bridge"))
...
fn read_or_create_install_identity(path: PathBuf) -> String {
    ...
    let seed = format!(
        ...
        now_string(),
        std::process::id(),
        env::var("PANDA_BRIDGE_INSTALL_ID_SEED").unwrap_or_default()
    );
...
pub(crate) fn local_computer_name() -> Option<String> {
    env::var("PANDA_BRIDGE_COMPUTER_NAME")
        .ok()
...
fn local_device_model() -> String {
    env::var("PANDA_BRIDGE_DEVICE_MODEL")
        .ok()
        .and_then(|value| sanitize_display_field(&value, 80))
