...
pub(crate) fn preview_intent(api: &str, intent: &str) -> Result<IntentPreview, String> {
    ...
    Ok(IntentPreview {
        ...
        device_name: payload
            .connect_intent
            .device_name
            .unwrap_or_else(|| "Bridge Desktop".to_string()),
        user_id: payload
            .connect_intent
            .user
            .as_ref()
...
pub(crate) fn claim_intent_pending(
    ...
    let body = json!({
        "device_name": if device_name.trim().is_empty() { "Bridge Desktop" } else { device_name.trim() },
        "app_version": VERSION,
    ...
    for (index, product_ids) in matching {
        ...
        for actual_product_id in &product_ids {
            ...
            let remote_revoke: Result<Value, String> =
                if env_flag("BRIDGE_SKIP_REMOTE_REVOKE") {
