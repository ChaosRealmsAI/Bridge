...
pub(crate) fn handle_verify_stream(
    ...
    let provided = headers
        .iter()
        .find(|(key, _)| key == "x-bridge-verify-token")
...
pub(crate) fn write_builtin_screenshot(path: &Path, snapshot: &Value) -> Result<(), String> {
    ...
    canvas.draw_text(34, 28, "BRIDGE DESKTOP", [255, 255, 255], 4);
    canvas.draw_text(
        34,
        104,
...
pub(crate) fn activate_desktop_app() -> Result<(), String> {
    if cfg!(target_os = "macos") {
        let status = Command::new("osascript")
            .args(["-e", "tell application \"Bridge\" to activate"])
            .status()
            .map_err(|error| error.to_string())?;
        if status.success() {
            return Ok(());
...
pub(crate) fn verify_control_state_path() -> Result<PathBuf, String> {
    if let Ok(path) = env::var("BRIDGE_VERIFY_CONTROL_STATE") {
