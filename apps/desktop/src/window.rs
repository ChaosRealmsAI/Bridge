...
pub(crate) fn desktop_ui_html() -> String {
    include_str!("../ui/index.html")
        .replace(
            "__BRIDGE_DESKTOP_CSS__",
            include_str!("../ui/styles.css"),
        )
        .replace(
            "__BRIDGE_DESKTOP_ABOUT_JS__",
            include_str!("../ui/about.js"),
        )
        .replace("__BRIDGE_DESKTOP_JS__", include_str!("../ui/app.js"))
}
    ...
    #[cfg(windows)]
    {
        ...
        if should_register_windows_url_scheme_on_startup() {
            if let Err(error) = register_windows_url_scheme() {
                eprintln!("[windows] failed to register bridge URL scheme: {error}");
            }
        }
    }
        ...
        *control_flow = ControlFlow::Wait;
        match event {
            ...
            Event::UserEvent(UserEvent::Respond { id, ok, payload }) => {
                ...
                let _ = webview.evaluate_script(&format!("window.Bridge.receive({});", message));
            }
...
pub(crate) fn register_windows_url_scheme() -> Result<(), String> {
    ...
    let scheme = windows_registry::CURRENT_USER
        .create(r"Software\Classes\bridge")
        .map_err(|error| error.to_string())?;
    scheme
        .set_string("", "URL:Bridge Protocol")
        .map_err(|error| error.to_string())?;
    scheme
        .set_string("URL Protocol", "")
        .map_err(|error| error.to_string())?;
    let command_key = windows_registry::CURRENT_USER
        .create(r"Software\Classes\bridge\shell\open\command")
        .map_err(|error| error.to_string())?;
    command_key
        .set_string("", command)
        .map_err(|error| error.to_string())
