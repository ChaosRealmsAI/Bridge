...
import { deviceRelayKeyExchange, normalizeRelayKeyBootstrap, plaintextRelayKeyFields, publicAccount, publicConnectIntent, publicAuthorization, publicDevice, publicRelayKeyBootstrap, publicStateDevice, publicStateProduct, updateAuthorizationRelayKeyBootstrap } from "./public-payloads.js";
...
export async function createDelegatedConnectIntent(request, env, productId) {
  ...
  const deviceName = clean(body.device_name || body.deviceName, 120) || "Bridge Desktop";
...
export async function requireProductDelegation(request, env, product, rawBody) {
  ...
  const productId = clean(request.headers.get("x-bridge-product-id"), 80);
  const userId = clean(request.headers.get("x-bridge-user-id"), 120);
  const deviceId = clean(request.headers.get("x-bridge-device-id"), 120);
  const timestamp = clean(request.headers.get("x-bridge-request-timestamp"), 80);
  const nonce = clean(request.headers.get("x-bridge-request-nonce"), 120);
  const bodyHash = clean(request.headers.get("x-bridge-body-sha256"), 80);
  const signature = clean(request.headers.get("x-bridge-signature"), 160);
