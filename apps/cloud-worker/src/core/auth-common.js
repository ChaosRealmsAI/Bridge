...
import { authorizationEpoch, boundedInteger, clean, cookie, httpError, normalizeEmail, now, object, randomToken, sha256Hex } from "./utils.js";
...
export async function productScopedUserId(productId, externalUserId) {
  ...
  const hex = await sha256Hex(`bridge:delegated-user:v1:${productId}:${scoped}`);
...
export function connectorInstallId(request) {
  return clean(request.headers.get("x-bridge-install-id"), 200);
}
...
export async function createDeviceWithToken(env, userId, input) {
  ...
  const patch = {
    device_name: clean(input.device_name, 120) || existing?.device_name || "Bridge Desktop",
    status: "online",
