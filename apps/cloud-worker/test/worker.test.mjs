...
async function delegatedApiRaw(method, path, body, userId, deviceId, nonce = randomUUID()) {
  ...
  return apiRaw(method, path, body, "", {
    "x-bridge-product-id": "delegated-demo",
    "x-bridge-user-id": userId,
    "x-bridge-device-id": deviceId,
    "x-bridge-request-timestamp": timestamp,
    "x-bridge-request-nonce": nonce,
...
assert.equal(health.protocol, "bridge-protocol-v0.2");
...
assert.equal(diagnostics.protocol, "bridge-protocol-v0.2");
...
assert.equal(diagnostics.install.version, "0.1.1");
assert.equal(diagnostics.install.download_path, "/downloads/bridge-macos.dmg");
assert.equal(diagnostics.install.versioned_download_path, "/downloads/releases/v0.1.1/bridge-desktop-v0.1.1-macos.dmg");
assert.equal(diagnostics.install.targets.macos.file_name, "bridge-macos.dmg");
assert.equal(diagnostics.install.targets.windows_x64.file_name, "bridge-windows-x64.zip");
assert.equal(diagnostics.install.release_manifest_url, "https://assets.bridge.chaos-realms.cc/downloads/bridge-desktop/latest.json");
...
const assetHead = await worker.fetch(new Request("http://local.test/downloads/bridge-macos.dmg", { method: "HEAD" }), env);
assert.equal(assetHead.status, 200);
assert.deepEqual(assetRequests.at(-1), { method: "HEAD", pathname: "/downloads/bridge-macos.dmg" });
const htmlDownloadFallback = await worker.fetch(new Request("http://local.test/downloads/bridge-windows-x64.zip"), {
  ...env,
  ASSETS: {
...
const wrongSelfhostPairingToken = await apiMissingOrigin("POST", "/v1/connectors/claim", {
  ...
  "x-bridge-local-client": "desktop",
  "x-bridge-install-id": "install-selfhost-wrong",
});
...
const selfhostPairing = await apiMissingOrigin("POST", "/v1/selfhost/pairing-token", {
  ...
  "x-bridge-selfhost-admin-token": "selfhost-admin-test",
});
...
const selfhostClaim = await apiMissingOrigin("POST", "/v1/connectors/claim", {
  ...
  "x-bridge-local-client": "desktop",
  "x-bridge-install-id": "install-selfhost-claim",
});
...
const reusedSelfhostPairing = await apiMissingOrigin("POST", "/v1/connectors/claim", {
  ...
  "x-bridge-local-client": "desktop",
  "x-bridge-install-id": "install-selfhost-reuse",
});
...
const expiredSelfhostClaim = await apiMissingOrigin("POST", "/v1/connectors/claim", {
  ...
  "x-bridge-local-client": "desktop",
  "x-bridge-install-id": "install-selfhost-expired",
});
...
const invalidTokenClaim = await apiMissingOrigin("POST", "/v1/connect-intents/pbi_missing/claim", {
  ...
  "x-bridge-local-client": "desktop",
  "x-bridge-install-id": "install-invalid-token",
});
...
const expiredTokenClaim = await apiMissingOrigin("POST", `/v1/connect-intents/${encodeURIComponent(expiredIntent.token)}/claim`, {
  ...
  "x-bridge-local-client": "desktop",
  "x-bridge-install-id": "install-expired-token",
});
...
const alreadyClaimed = await apiMissingOrigin("POST", `/v1/connect-intents/${encodeURIComponent(intent.token)}/claim`, {
  ...
  "x-bridge-local-client": "desktop",
  "x-bridge-install-id": "install-relay-test-duplicate",
});
...
const confirmed = await nativeConfirmIntent(intent.token, claimed.device_token, {
  "x-bridge-install-id": "install-relay-test",
});
...
const secondConfirmed = await nativeConfirmIntent(secondIntent.token, secondClaim.device_token, {
  "x-bridge-install-id": "install-relay-test-two",
});
...
const burnConfirmed = await nativeConfirmIntent(burnIntent.token, burnClaim.device_token, {
  "x-bridge-install-id": "install-burn-full",
});
...
const burnRelayOnlyConfirmed = await nativeConfirmIntent(burnRelayOnlyIntent.token, burnRelayOnlyClaim.device_token, {
  "x-bridge-install-id": "install-burn-relay-only",
});
...
const delegatedConfirmed = await nativeConfirmIntent(delegatedIntent.token, delegatedClaim.device_token, {
  "x-bridge-install-id": "install-delegated-demo",
});
